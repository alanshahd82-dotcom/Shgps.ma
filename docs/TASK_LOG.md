...
## ثغرة اليقين في مراقبة الأسطول — فصل المحرك مقابل فصل البطارية

### ما الذي كان غير دقيق

- نجاح `POST /devices/:id/command` كان يعني أن Traccar قبل الأمر أو وضعه في
  الطابور، وليس أن relay نفّذ القطع فعليًا على السيارة. كما أن الواجهة كانت
  تغيّر `engineOn` مباشرة بعد الإرسال، فتعرض نجاحًا غير مثبت.
- `GET /devices` و`GET /devices/:id` و`GET /map/positions` كانت تستنتج
  `powerDisconnected` من position قديمة + voltage مخزّن. هذا الخلط يعيد
  إشعار/عرض فصل بطارية عند فقد الشبكة أو نوم الجهاز، حتى بعد إصلاح مسار
  التحقق المركزي.
- غياب `ignition` أو غياب position حديثة كان يتحول إلى `engineOn:false`،
  مع أن الحالة الصحيحة هي «غير معروفة».

### ما تم توحيده

- الصمت لا يساوي فصل بطارية في أي endpoint؛ `powerDisconnected` يظهر فقط
  من إشارة tracker حديثة وصريحة أو من episode تم تأكيده مركزيًا بعد تحقق
  Traccar.
- API المحرك يعيد الآن `executionConfirmed:false` و
  `confirmationState:'pending_telemetry'` بعد قبول/طابور الأمر. الواجهة لا
  تغيّر حالة المحرك تفاؤليًا، وتنتظر packet حديثة فيها ignition.
- عند عدم وجود ignition موثوق، تفاصيل أوامر المحرك تعرض «حالة المحرك غير
  معروفة» وتعطل الزر بدل اختيار start/stop عشوائيًا.
- هذا التفريق مركزي وينطبق على جميع الأجهزة والحسابات، لا على bekane أو
  DACIA وحدهما.

### حدود ما يمكن معرفته من الجهاز

- لا يمكن للبرنامج أن يثبت «تم قطع المحرك فعليًا» من HTTP 200 أو command ID؛
  يلزم feedback من tracker (مثل ignition/relay state) بعد الأمر.
- لا يمكن إثبات «فصل بطارية المركبة» من غياب packet وحده؛ الغياب قد يكون
  تغطية GSM أو tracker نائمًا أو عطلًا. إذا كان الجهاز لا يرسل external-power
  صريحًا، تعرض المنصة الحالة كغير مؤكدة ولا تخترع سببًا.
