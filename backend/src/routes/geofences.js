...
geofencesRouter.get('/', requireAuth, async (req, res) => {
  try {
    ...
    const { rows: devRows } = await db.query('SELECT traccar_id FROM devices WHERE user_id=$1', [req.user.id])
    if (devRows.length === 0) return res.json([])
    const results = await Promise.all(devRows.map(d => traccar.getGeofencesByDevice(d.traccar_id).catch(() => [])))
    const seen = new Set()
    const merged = results.flat().filter(g => { if (seen.has(g.id)) return false; seen.add(g.id); return true })
    return res.json(merged)
  } catch (err) { console.error('[geofences GET]', err.message); res.status(500).json({ error: 'Failed to fetch geofences' }) }
})
  ...
  try {
    ...
    const { rows } = await db.query(
      `INSERT INTO local_geofences (user_id, device_id, name, type, coords, radius, notify_enter, notify_exit)
       VALUES ($1, $2, $3, 'circle', $4, $5, $6, $7) RETURNING *`,
      [req.user.id, dbDeviceId, name.trim(), coords, Number(radius), notifyEnter !== false, notifyExit !== false]
    )
    let syncFailed = false
    try { await traccar.createGeofence(name.trim(), center.lat, center.lng, Number(radius)) }
    catch (syncErr) { console.warn('[geofences POST] Traccar sync failed:', syncErr.message); syncFailed = true }
    res.status(201).json({ ...rows[0], syncFailed })
  } catch (err) { console.error('[geofences POST]', err.message); res.status(500).json({ error: 'Failed to create geofence' }) }
})
...
geofencesRouter.delete('/:id', requireAuth, async (req, res) => {
  ...
  } catch (err) { console.error('[geofences DELETE]', err.message); res.status(500).json({ error: 'Failed to delete geofence' }) }
})
