...
sharingRouter.post('/', requireAuth, async (req, res) => {
  try {
    ...
    await db.query('INSERT INTO share_links (token, device_id, expires_at) VALUES ($1,$2,$3)', [token, deviceId, expiresAt])
...
sharingRouter.get('/:token', async (req, res) => {
  try {
    ...
    res.json({
      deviceName: link.name, plate: link.plate, type: link.type, expiresAt: link.expires_at,
      position: position ? { lat: position.latitude, lng: position.longitude, speed: position.speed, fixTime: position.fixTime } : null,
    })
  } catch (err) { console.error(err); res.status(500).json({ error: 'Server error' }) }
})
