...
adminRouter.get('/stats', requireAdmin, async (req, res) => {
  try {
    const [clients, devices, alerts, revenue, expiring, unactivated] = await Promise.all([
      db.query('SELECT COUNT(*) FROM users WHERE is_admin=false AND is_active=true'),
    ...
    // Online devices from Traccar
    let onlineCount = 0
    try {
      const positions = await traccar.getAllPositions()
      onlineCount = positions.length
    } catch {}
...
// GET /api/admin/stats/revenue — monthly revenue chart
adminRouter.get('/stats/revenue', requireAdmin, async (req, res) => {
  try {
    const { rows } = await db.query(`
      ...
      WHERE s.created_at >= NOW() - INTERVAL '7 months'
...
// GET /api/admin/reports?clientId=&from=&to=
adminRouter.get('/reports', requireAdmin, async (req, res) => {
  try {
    ...
    if (clientId) {
      const { rows } = await db.query(
        'SELECT d.*, u.name AS client_name FROM devices d JOIN users u ON d.user_id=u.id WHERE d.user_id=$1',
        [clientId]
      )
      deviceRows = rows
    } else {
      const { rows } = await db.query(
        'SELECT d.*, u.name AS client_name FROM devices d JOIN users u ON d.user_id=u.id ORDER BY u.name'
      )
      deviceRows = rows
    }
    ...
    for (const dev of deviceRows) {
      ...
      if (dev.traccar_id) {
        try {
          const t = await traccar.getTrips(dev.traccar_id, fromDate.toISOString(), toDate.toISOString())
          trips   = t.length
          totalKm = t.reduce((s, tr) => s + tr.distance / 1000, 0)
        } catch {}
      }
...
adminRouter.patch('/settings', requireAdmin, async (req, res) => {
  try {
    ...
    if (company) {
      await db.query(
        `INSERT INTO system_settings (key, value) VALUES ('company', $1)
    ...
    if (plans) {
      await db.query(
        `INSERT INTO system_settings (key, value) VALUES ('plans', $1)
         ON CONFLICT (key) DO UPDATE SET value=$1`,
