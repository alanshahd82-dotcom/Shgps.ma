...
import {
  ChevronLeft, Plus, Cpu, Wifi, WifiOff, X,
...
import { getVoltageColor, VehicleIcon, VehicleTypeControl } from '../../components/ui'
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  { icon: Zap, label: isAr ? 'الفولطاج' : 'Tension', val: live.status === 'online' && Number(live.voltage) > 0 ? `${Number(live.voltage).toFixed(1)} V` : (isAr ? 'مفصول' : 'Déconnecté') },
                ...
                >
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <div className="flex items-center gap-1 text-xs">
                      ...
                      <span style={{ color: getVoltageColor(device.voltage) }} className="font-semibold">{Number(device.voltage) > 0 ? `${Number(device.voltage).toFixed(1)} V` : (isAr ? 'مفصول' : 'Déconnecté')}</span>
                      {device.batteryLevel != null && <span className="text-[10px] text-slate-400">{Math.round(Number(device.batteryLevel))}%</span>}
                    </div>
