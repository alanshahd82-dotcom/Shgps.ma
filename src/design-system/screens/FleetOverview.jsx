...
function VehicleQuickLink({ vehicle, onClick }) {
  ...
  return (
    <button
      ...
      aria-label={`فتح مركبة ${vehicle.name}`}
    >
      ...
      <span className="min-w-0">
        ...
        <span className="block text-[10px] text-slate-500">{hasAlert ? 'تحتاج إلى مراجعة' : moving ? 'متحركة الآن' : 'عرض التفاصيل'}</span>
      </span>
...
export function FleetOverview() {
  ...
  const { alertsList, unreadCount } = useApp()
  ...
  return (
    <section className="pointer-events-none absolute inset-x-3 top-3 z-[500] space-y-2 sm:inset-x-5 sm:top-4" dir="rtl" aria-label="ملخص الأسطول">
      <div className="pointer-events-auto flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <Metric icon={Car} label="المركبات" value={summary.total} />
        {summary.connected !== null && <Metric icon={Radio} label="متصلة" value={summary.connected} tone="connected" />}
        {summary.disconnected !== null && <Metric icon={WifiOff} label="غير متصلة" value={summary.disconnected} tone="offline" />}
        {summary.moving !== null && <Metric icon={Gauge} label="متحركة" value={summary.moving} tone="moving" />}
        {hasAlertData && <Metric icon={AlertTriangle} label="تنبيهات" value={unreadCount} tone={hasAlerts ? 'alert' : 'neutral'} />}
      </div>
      ...
      {hasAlerts && (
        <button
          ...
          aria-label={`فتح ${unreadCount} تنبيهات غير مقروءة`}
        >
          <AlertTriangle className="h-4 w-4 shrink-0" aria-hidden="true" />
          <span className="truncate">{unreadCount} تنبيهات غير مقروءة</span>
