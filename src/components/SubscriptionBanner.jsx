...
export default function SubscriptionBanner({ device, lang = 'ar', onRenew, dark = false }) {
  ...
  return (
    <div className={`flex items-start gap-3 rounded-2xl p-4 border ${
      dark
        ? expired ? 'bg-red-400/10 border-red-400/25 text-white' : 'bg-orange-400/10 border-orange-400/25 text-white'
        : expired ? 'bg-red-50 border-red-100 text-red-800' : 'bg-orange-50 border-orange-100 text-orange-800'
    }`}>
      <AlertTriangle size={18} className={`flex-shrink-0 mt-0.5 ${expired ? 'text-red-400' : 'text-orange-400'}`} />
