...
import { ADMIN_EMAIL, ADMIN_PHONE } from '../../config/contact'
...
function RenewalActions({ device, lang, onClose }) {
  ...
  const whatsappNumber = ADMIN_PHONE.replace(/\D/g, '')
  const whatsappUrl = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(message)}`
  const subject = isAr ? `تجديد اشتراك Athar GPS — ${device.name || 'جهاز'}` : `Renouvellement Athar GPS — ${device.name || 'appareil'}`
  const mailtoUrl = `mailto:${ADMIN_EMAIL}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(message)}`
    ...
    <div className="mt-3 rounded-xl border border-[rgba(224,179,111,.18)] p-3" style={{ background: 'rgba(224,179,111,.06)' }}>
      ...
      <div className="grid grid-cols-2 gap-2">
        <a
          ...
          style={{ background: '#25D366', color: '#052b15' }}
        >
          <Smartphone size={14} />
          WhatsApp
        </a>
        <a
          href={mailtoUrl}
          className="flex items-center justify-center gap-1.5 rounded-lg px-2 py-2 text-[10px] font-black"
          style={{ background: 'rgba(140,163,184,.16)', color: 'var(--ath-txt)' }}
        >
          <Mail size={14} />
          Email
        </a>
...
function SubscriptionCard({ device, lang, renewOpen, onRenew }) {
  const isAr = lang === 'ar'
  ...
  return (
    <article className="ath-card p-3.5" style={{ color: 'var(--ath-txt)' }}>
      ...
      {renewOpen && <RenewalActions device={device} lang={lang} onClose={onRenew} />}
    </article>
  )
}
