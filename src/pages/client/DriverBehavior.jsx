...
import { Gauge, AlertTriangle, ChevronDown, Car, BarChart2 } from 'lucide-react'
  ...
  return (
    <div className="client-app min-h-screen bg-[#f5f7f8] dark:bg-[#0b1524] pb-28" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <div className="px-5 pt-5 pb-4">
        <h1 className="text-primary-500 font-extrabold text-xl mb-4">
        ...
        <button
          ...
          aria-label={isAr ? 'اختر الجهاز' : 'Choisir un appareil'}
            ...
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
              ...
              {devices.map(d => (
                <button key={d.id} onClick={() => { setDeviceId(String(d.id)); setShowDevices(false) }}
                  className="w-full px-4 py-3 text-left text-sm dark:border-slate-700"
        ...
        {/* Period tabs */}
        <div className="flex gap-2" role="group" aria-label={isAr ? 'اختر الفترة' : 'Choisir la période'}>
          {PERIODS.map(p => (
      ...
      {loading ? (
        ...
        /* ── Empty state ── */
        <div className="mx-5 mt-4 p-8 rounded-2xl flex flex-col items-center gap-4 bg-white dark:bg-[#112240] border border-slate-200 dark:border-slate-700 shadow-sm">
          ...
          <div className="text-center">
            ...
            <p className="text-slate-400 text-xs mt-1">
              {isAr
                ? 'ستظهر بيانات السلوك بعد أول رحلة مسجّلة'
                : 'Les données apparaîtront après le premier trajet enregistré'}
            </p>
          </div>
        </div>
      ) : (
            ...
            <div className="space-y-2">
              {tips.map((tip, i) => (
                <div key={i} className="flex items-start gap-3 p-3.5 rounded-2xl"
                  ...
                  <p className="text-xs leading-relaxed text-slate-700 dark:text-slate-300">{tip}</p>
                </div>
              ))}
            </div>
          )}
