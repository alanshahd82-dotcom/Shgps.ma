...
function locale(isAr) { return isAr ? 'ar-MA' : 'fr-FR' }

export function formatDateTZ(value, isAr, opts = {}) {
  const date = value instanceof Date ? value : new Date(value)
  if (Number.isNaN(date.getTime())) return ''
  return date.toLocaleDateString(locale(isAr), { timeZone: APP_TZ, ...opts })
}

export function formatTimeTZ(value, isAr, opts = {}) {
  const date = value instanceof Date ? value : new Date(value)
  if (Number.isNaN(date.getTime())) return ''
  return date.toLocaleTimeString(locale(isAr), { timeZone: APP_TZ, hour: '2-digit', minute: '2-digit', ...opts })
}
